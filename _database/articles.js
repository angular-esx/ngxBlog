export var ARTICLES = _init();

function _init() {
  var _articles = {}; 

  for (var i = 1; i <= 10; i++) {
    _articles[i] = new _article(
      i,
      'Article ' + i,
      new Date(),
      'blog/resources/images/xblog-home-cover.jpg',
  ...
  for (var articleId in _articles) {
    _articles[articleId].relatedArticles = [
      _articles[1],
      _articles[2],
      _articles[3],
      _articles[4],
...
function _article(id, title, postedDate, cover, description, content) {
  this.id = id;
  this.title = title;
  ...
  this.routeLink = '/blog/articles/' + title.toLowerCase().trim().replace(new RegExp(' ', 'g'), '-') + '-' + id + '.html';
}
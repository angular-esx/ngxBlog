...
export var xblogCodePanelComponent = Component({
  ...
  styles: [":host(.xblog-code-panel){display:block;box-shadow:0 1px 4px 0 rgba(0,0,0,0.14)}:host(.xblog-code-panel) /deep/>.ngx-card{border-radius:0;border:none}:host(.xblog-code-panel) /deep/>.ngx-card xblog-title{font-weight:800;color:#333}:host(.xblog-code-panel) /deep/>.ngx-card a[xblog-source-code]{font-style:italic;font-weight:800;text-decoration:none;text-transform:lowercase}:host(.xblog-code-panel) /deep/>.ngx-card a[xblog-source-code]:focus,:host(.xblog-code-panel) /deep/>.ngx-card a[xblog-source-code]:hover{color:#5cb75c}:host(.xblog-code-panel) /deep/>.ngx-card ngx-card-content{margin:0.5rem -1.25rem -3.5rem -1.25rem;color:#fff;background-color:#3C4556}:host(.xblog-code-panel) /deep/>.ngx-card ngx-card-content code{display:block;overflow-x:auto;margin:-1.5rem 0}:host(.xblog-code-panel) /deep/>.ngx-card ngx-card-content code xblog-code{display:block;font-size:.875rem;padding:1rem 1.25rem}:host(.xblog-code-panel).xblog-code-panel-type-no-header /deep/>.ngx-card{padding-top:0}:host(.xblog-code-panel).xblog-code-panel-type-no-header /deep/>.ngx-card>.ngx-card-header{display:none}"],
  inputs: [ 'type' ],
  host: {
